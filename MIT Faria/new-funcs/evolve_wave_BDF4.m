function [phi_hat, eta_hat, psi_hat] = evolve_wave_BDF4(phi_hat, eta_hat,psi_hat,t_in,p, type)
% Evolves wave field in F-space through nsteps of size dt
debug0 = 0;

if type == 0
    [phi_hat,eta_hat,psi_hat] = evolve_wave(phi_hat,eta_hat,psi_hat, t_in,p);
    return
end
...
temp_vec = [phi_hat(:); eta_hat(:)]; %[vec(phi_hat); vec(eta_hat)];
...
for n=1:min(2*smallN,smallN*p.nsteps_impact)
    ...
    RHS1_1 = - 0*p.g(t)*eta_hat;
    RHS2_1 = phi_hat;

    RHS1_2 = DtN_new(phi_hat,p);
    RHS2_2 = eta_hat;
    

    Mtime_big = [O_big, eps*p.g(t)*I_big; O_big, O_big];

    temp_vec = (M_big + Mtime_big)\([RHS2_1(:); RHS2_2(:)]);
    phi_hat = reshape(temp_vec(1:end/2).', p.Ny, p.Nx);
    eta_hat = reshape(temp_vec(end/2+1:end).', p.Ny, p.Nx);
    eta_hat = eta_hat + eps*DtN_new(phi_hat,p);
    t = t+p.dt/smallN;
    if ( n == smallN && type > 1)
        phi_hat1 = phi_hat;
        eta_hat1 = eta_hat;
...
for n=3:p.nsteps_impact  
    ...
    RHS1_1 = - 0*p.g(t)*eta_hat;
    if type > 1
        RHS2_1 = c1*phi_hat + c2*phi_hat2 + c3*phi_hat1 + c4*phi_hat0;
        phi_hat0 = phi_hat1;
        phi_hat1 = phi_hat2;
        phi_hat2 = phi_hat;
    else
    ...
    RHS1_2 = DtN_new(phi_hat,p);
    if type > 1
        RHS2_2 = c1*eta_hat + c2*eta_hat2 + c3*eta_hat1 + c4*eta_hat0;
        eta_hat0 = eta_hat1;
        eta_hat1 = eta_hat2;
        eta_hat2 = eta_hat;
    else
